...
----
# Class vowel space: F2024 edition
```{r echo=FALSE}
library(ggplot2)
data_24 <- read.csv("LING2200_vowels_2024.csv", header=TRUE)
vowel_space_24 <- ggplot(data_24, aes(x=F2, y=F1, label=Vowel)) + geom_text() + stat_ellipse(aes(group = Vowel)) + scale_x_reverse() + scale_y_reverse() + theme_minimal()
vowel_space_24
```

As you can see, we're pretty similar. There are some obvious measurement errors (look at that one [i] east of [u] territory!). Schwas are west of "hot", but close. In that case, what's going on with perception? How do we tell if someone is saying "hut" or "hot"? Are there secondary cues?

# Models of production
 
 When we say "model" what do we mean? A model is a simplification of what we think is actually happening. Much like a model of a train, a model of some behaviour is approximates many complexities, which are put into a perspective that is tractable, yet captures the major features of the *actual thing*. It allows us to better understand complex systems and behaviours.
