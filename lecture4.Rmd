...
1. From the Praat Objects window > New > Sound > Create sound from formula\
...
3. The most important field is "Formula." Notice it says "1/2 * sin(2 * pi * 377 * x)" by default (the spaces don't matter). It also says "+ randomGauss(0,0.1)", which you can select and delete. What that does is add random noise to the pure tone signal. Change the Frequency to 350 (from 377)

4. Compare the formula in Formula field to the one above. Notice the similarities? The default formula has A = 1/2 and a frequency of 350Hz. (You can change the frequency to anything you want). Name the object "350Hz."

5. This will put an object called "350Hz" in your Objects window.
...
7. The waveform view doesn't really look like a sine waveform like we're used to seeing. That's because it's repeating itself 350 times per second and the window is showing 3 seconds (or 350 x 3) 1050 cycles. So, let's zoom in to the waveform to see the individual contours of the waveform.
...
11. Suppose you want to place the beginning of your cursor at the true starting point of a cycle. This would be the so-called "zero crossings" of the wave. The arrows in the example above are pointing the the onset and offset of one cycle of the wave, each at the zero crossing. You can get the cursor exactly at the zero crossing by going to Sound > "Move start of selection to nearest zero crossing", and then do the same for the end of the selection. 

12. Can you confirm whether the synthesized tone is actually 350Hz? $\rightarrow$ remember, the frequency is the number of cycles per second.
...
Notice that in the bottom image the curve is skinnier with a sharp peak (centered at 350Hz). This is because the program had more "evidence" for the nature of the frequency. It's basically computed the average frequency over 3s, while in the top image, it is only computing the frequency at a single point (which is the center of a 10ms window).
...
Voila! It looks like the wave has 10 periods in 0.05s, so 10/0.05 = 200Hz! So if you play this sound, it will sound complex, but it will have the same pitch of a pure 200Hz tone.
