...
The first week of class we dive into our subject matter by introducing some basic concepts underlying the acoustics of speech. In order to get to there, however, we need to begin with some definitions of concepts from physics, which we will rely on for the rest of the course. [Ch.1](ch1-Sound.pdf)

***

# The Speech Chain
Take a look at the image below, the so-called "speech chain." It's an image that's used in many courses on language and cognitive science. It represents the nature of communicating a linguistic message via the vocal-audition apparatus. The image captures a lot of the basic ideas that we will be exploring in this course. 
<br>
<br>
<p align="center">
...
We will be dealing with all of these topics at a fairly basic or introductory level of sophistication. That means that we're only scratching the surface and each of these topics can be their own class! It's also important to note that our course will not be immediately concerned with **linguistic** representations and abstractions, topics which will be covered by other courses offered in the program like Phonetics (LING 2110), Acoustic Phonetics (LING 4220) and Phonology (LING 2120 and 3120). For example, we won't be looking at sets of data from different languages in order to figure out their sound system of phonological rules. We'll be dealing with concepts that are universal to all languages, and in cases where we have to a specific language, that language will likely be North American English. One thing to keep in mind is that we are going to be dealing with the physical instantiation of **spoken** language. You should know that human language comes in different flavors, that is, we can use different mechanical systems provided by the human body to communicate ideas. Normally developing humans ultimately utilize the oral (mouth)-aural (ear) system. If you acquire or are born with hearing loss, it is likely that you would use a manual (signs) system for communicating language. 

Back to the speech chain: in order to understand the fundamentals of the science of speech production, we have to have some grounding in the rules that govern the physical world. We will start with some terminology and definitions. 
...
The physical concept of **pressure** is particularly important for our understanding of speech processes. We will rely on our understanding of pressure to explain both audition as well as the production of speech sounds.\
...
Pressure **differentials**, or the change in pressure over time across some area or between two spaces, will play a very important role in our understanding of how we produce and hear sounds.

# The story of sound

When we talk about "sound" what do we actually mean? A good description of sound might be that it is anything that we *hear*. But we know, intuitively, that sound is something more than its connection to hearing. A good starting point is to examine what it takes to make a sound. You'll notice that all sounds result from the *movement* of an object. You hear a bee buzzing because its wings are rapidly flapping, etc. 
Sound is essentially changes in air pressure that results from some sort of disturbance, like the smacking of a table top or the vibrating of vocal folds. For speech sounds, tiny changes in pressure are a result of air molecules being set in motion by the movement of the vocal folds. These changes in air pressure between air molecules end up at the listener's ear. This animated gif schematizes the general idea of some movement (the hand moving up) setting in to motion the rope. The change in air pressure is represented by the wave in the rope, which moves and ultimately reaches a target (or the listener's ear).
...
Air molecules naturally spread themselves out equally. To get this equalization, air moves from an area high pressure to an area of low pressure → driving pressure. Air only moves when there is a *pressure differential* between the areas. If there is no difference in pressure between the two areas, there is no airflow.
...
2. **Inertia**: the tendency of an object in motion to stay in motion
...
Period is inversely proportional to frequency. So:
$$F = 1/t ; t = 1/F$$
